...
describe("rateLimit", () => {
  it("blocks a client after its quota and reports the deterministic reset", async (t) => {
    ...
    now += 1_000
    expect((await request(router, "10.0.0.1")).headers.get("Retry-After")).toBe("59")
  })

  it("isolates quotas by normalized client address and header precedence", async () => {
    let normalizations: Array<[Record<string, string>, Record<string, string>]> = [
      [
      ...
      [{ "true-client-ip": "[2001:db8::1]:443" }, { "true-client-ip": "2001:db8::1" }],
      [{ "x-forwarded-for": "::ffff:203.0.113.4" }, { "x-forwarded-for": "203.0.113.4:8080" }],
    ]
    ...
    for (let [firstHeaders, equivalentHeaders] of normalizations) {
      ...
      expect((await request(router, undefined, equivalentHeaders)).status).toBe(429)
    }
  ...
  it("skips configured paths and loopback requests only", async () => {
    ...
    expect((await request(router, "10.0.0.5", {}, "/healthcheck")).status).toBe(200)
    expect((await request(router, "10.0.0.5", {}, "/healthcheck")).status).toBe(200)
    expect((await request(router, undefined, {}, "/", "localhost")).status).toBe(200)
    expect((await request(router, undefined, {}, "/", "localhost")).status).toBe(200)
    expect((await request(router, undefined, {}, "/", "example.com")).status).toBe(200)
    expect((await request(router, undefined, {}, "/", "example.com")).status).toBe(429)
  })
...
function request(
  ...
  return router.fetch(new Request(`http://${hostname}${pathname}`, { headers }))
}
