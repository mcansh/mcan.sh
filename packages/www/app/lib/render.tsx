...
import { getCSPNonce } from "./context.ts"
...
export function render(node: Remix.RemixNode, init?: ResponseInit) {
	...
	let stream = renderToStream(node, {
		resolveFrame(src) {
			return resolveFrame(request, src)
		},
...
export async function renderDocument(
	...
	{ bodyClassName, ...init }: ResponseInit & { bodyClassName?: string } = {},
) {
	let nonce = getCSPNonce()

	return render(
		<Document nonce={nonce} bodyClassName={bodyClassName}>
			{node}
		</Document>,
		init,
	)
